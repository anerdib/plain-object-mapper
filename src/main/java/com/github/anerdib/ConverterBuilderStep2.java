...
public class ConverterBuilderStep2<S, D, I> {
	...
	public ConverterBuilderStep2(Class<S> sourceClass, Class<D> destinationClass, Class<I> intermediaryClass, Supplier<I> factoryMethod, Function<I, D> finalizer) {
	...
	public final Converter<S, D> build() {
		...
		for (Method method : sourceClass.getMethods()) {
			...
			try {
				...
				Method setter = destinationClass.getMethod(setterName, new Class<?>[]{method.getReturnType()});
				if (setter != null) {
					...
					Setter setterLambda = (i, p) -> {
						try {
							...
							log.log(Level.SEVERE, "setter invocation failed for {} in class {}",
									new Object[]{setterName, destinationClass.getName()});
						}
					};
